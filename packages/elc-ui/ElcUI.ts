import bootstrapTemplateHtml = require("dojo/text!./Templates/elc-ui-bootstrap.min.html");
import templateHtml = require("dojo/text!./Templates/elc-ui.min.html");
import Route from "wsdot-elc/Route";
import { IRouteLocation } from "wsdot-elc/RouteLocationInterfaces";
...
export default class ElcUI {
    ...
    constructor(rootNode: HTMLElement, options: IElcUIOptions) {
        ...
        const doc = parser.parseFromString(options && options.bootstrap ? bootstrapTemplateHtml : templateHtml, "text/html");
        const uiDom = (doc.body.querySelector(".elc-ui-root") as Element).cloneNode(true) as HTMLElement;
        this.root = rootNode;
        this.root.innerHTML = uiDom.outerHTML;

        // Setup route selector

        this.routeSelector = new RouteSelector(this.root.querySelector(".route-selector") as HTMLElement);

        // Setup nearest route location form
        const form = self.root.querySelector(".find-nearest-route-location-form") as HTMLFormElement;
        form.onsubmit = () => {
        ...
        const findRouteLocationForm = self.root.querySelector(".find-route-location-form") as HTMLFormElement;
        ...
        function changeMPMode() {
            const isSrmp = Boolean(findRouteLocationForm.querySelector("input[value=SRMP]:checked"));
            const isLine = Boolean(findRouteLocationForm.querySelector("input[value=line]:checked"));
        ...
        findRouteLocationForm.onsubmit = (submitEvent) => {
            ...
            const isSrmp = Boolean(inputForm.querySelector("input[value=SRMP]:checked"));
        ...
        findRouteLocationForm.addEventListener("reset", () => {
            const checkedRB = findRouteLocationForm.querySelectorAll("input[checked]");
        ...
        const radioButtons = findRouteLocationForm.querySelectorAll("input[type=radio]");
